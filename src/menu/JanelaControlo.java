...
public class JanelaControlo extends JFrame {
    ...
    private ArrayList<MarcadorPosto> marcadores = new ArrayList<MarcadorPosto>(); //a lista dos marcadores
    private MarcadorPosto marcadorAtual; //o marcador selecionado atualmente (null se não houver nenhum selecionado)
    private JList<Camiao> listaCamioes; //a lista de camiões

    /** imagem com o mapa */
    private static ImageIcon mapa = new ImageIcon("icones/mapa.jpg");
    private JPanel painelDesenho; //onde vai desenhar o mapa
    ...
    public JanelaControlo(Central c) {
        ...
        //criar os marcadores para os postos
    ...
    private void desenharPostos(Graphics2D g) {
        mapa.paintIcon(painelDesenho, g, 0, 0); //desenhar o mapa

        Point pos = central.getPosicao(); //puxa a posição da central
        ...
        //desenhar o itinerário de um ambiente especial (por causa das linhas)
        Graphics2D ge = (Graphics2D) g.create();

        //desenhar o itinerário do camião selecionado
        if (camiaoSel != null) {
            Itinerario iti = camiaoSel.getItinerario();
            //se houver um camião selecionado, desenha o seu itinerário

            if (iti != null) { //se houver um itinerário, desenha-o
                Point p1 = iti.getInicio(); //ponto inicial

                //se houver um itinerário, desenha-o
                for (int i = 0; i < iti.contarParagens(); i++) {

                    Point p2 = iti.paragem(i);
    ...
    private void desenhaLinha(Graphics2D ge, Point p1, Point p2) {
        ...
        //e outra mais fina e opaca, para dar um efeito "bonito"
   ...
   /** método chamado quado o utilizador preeeiona o botão de mudar o turno*/
    ...
    private void botaoEsquerdoPremido(MouseEvent e) {
        ...
        //escolheu um marcador válido, pedir a quatidade de litros
        ...
        String msg = null;
        switch (r) {
                //se o pedido foi aceite, não há nada a fazer
            case Central.EXCEDE_CAPACIDADE_CAMIAO:
                msg = "Camião não tem capacidade";
                break;
                //se o posto não tem capacidade, avisa o utilizador
            case Central.EXCEDE_CAPACIDADE_POSTO:
                msg = "Posto não tem capacidade";
                break;
                //se o posto não precisa de ser abastecido, avisa o utilizador
            case Central.POSTO_NAO_PRECISA:
                msg = "Posto não tem necessidade";
                break;
                //se o turno é longo demais, avisa o utilizador
        ...
        if (msg != null) {
            ...
            //se não deu erro, marcar o marcador como escolhido
    ...
    private JPanel getPainelComandos() {
        ...
        //adicionar os camiões à lista, para serem selecionados
